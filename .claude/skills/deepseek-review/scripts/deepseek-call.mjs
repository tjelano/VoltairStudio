...
async function main() {
  ...
  const providerEnv = process.env.DEEPSEEK_REVIEW_PROVIDER;

  /*
   * A typo (e.g. "openrotuer") falls through to the `!== "cheaperinference"`
   * check below and silently selects OpenRouter -- reject anything that
   * isn't unset or one of the two real provider names instead.
   */
  if (providerEnv !== undefined && providerEnv !== 'openrouter' && providerEnv !== 'cheaperinference') {
    console.error(
      `Unknown DEEPSEEK_REVIEW_PROVIDER: "${providerEnv}". Use "openrouter" or "cheaperinference", or unset it.`,
    );
    process.exitCode = 2;
  ...
  const useOpenRouter = providerEnv !== 'cheaperinference';
  ...
  if (!apiKey) {
    console.error(
      useOpenRouter
        ...
        : 'CHEAPERINFERENCE_API_KEY is not set in this shell.',
    );
    process.exitCode = 2;
  ...
  if (!/^https:\/\//i.test(baseUrl)) {
    console.error(
      `Refusing to send the API key to a non-HTTPS URL: ${baseUrl}. Check ${useOpenRouter ? 'OPENROUTER_BASE_URL' : 'CHEAPERINFERENCE_BASE_URL'}.`,
    );
    process.exitCode = 2;
  ...
  const models = useOpenRouter && model !== OPENROUTER_FALLBACK_MODEL ? [model, OPENROUTER_FALLBACK_MODEL] : undefined;
  ...
  if (flag === '--system' && existsSync(historyFile)) {
    console.error(
      `${historyFile} already exists, but --system was passed. Delete it first for a fresh Round 1, or omit --system to resume the existing thread.`,
    );
    process.exitCode = 2;
  ...
  if (flag !== '--system' && !existsSync(historyFile)) {
    console.error(
      `${historyFile} doesn't exist yet and no --system flag was given -- nothing to resume. Pass --system <system-file> for a fresh Round 1, or check the history-file path if you meant to resume.`,
    );
    process.exitCode = 2;
  ...
  if (existsSync(historyFile)) {
    ...
    try {
      ...
      /*
       * A pre-existing history file that's empty or incomplete (no system
       * prompt, or a dangling turn with no reply) would otherwise pass this
       * check and let the call proceed with no adversarial mandate -- the
       * same silent-no-system-prompt shape the checks above already guard
       * against. A complete round is always [system, user, assistant, ...] --
       * at least 3 entries, growing by 2 (user+assistant) per further round.
       */
      if (parsed.length < 3 || parsed[0]?.role !== 'system' || parsed[parsed.length - 1]?.role !== 'assistant') {
        throw new Error('history must start with a system message and end with a completed assistant turn');
      }

      history = parsed;
    } catch (e) {
      console.error(
        `${historyFile} exists but could not be read as valid JSON (${e.message}). Delete it to start a fresh review, or restore it from a backup.`,
      );
      process.exitCode = 2;
  ...
  try {
    const res = await fetch(baseUrl, {
      ...
      headers: {
        ...
        ...(useOpenRouter
          ? { 'HTTP-Referer': 'https://github.com/tjelano/VoltairStudio', 'X-Title': 'VoltairStudio deepseek-review' }
          : {}),
      },
      body: JSON.stringify({
        ...(models ? { models } : { model }),
    ...
    console.error(
      `DeepSeek call failed: ${e.name === 'TimeoutError' ? `no response after ${PROVIDER_TIMEOUT_MS / 1000}s` : e.message}`,
    );
    process.exitCode = 1;
